...
try:
    from .byfuid import ByfuidGenerator as RustByfuidGenerator
    RUST_AVAILABLE = True
except ImportError:
    RUST_AVAILABLE = False
    print("警告: Rust 扩展加载失败，使用纯 Python 实现", file=sys.stderr)
...
if not RUST_AVAILABLE:
    ...
    class ByfuidGenerator:
        ...
        def generate_custom_data(self, custom_input: Optional[str] = None, length: int = 201) -> str:
            if length != 201:
        ...
        def generate_byfuid(self, user_input: Optional[str] = None, custom_input: Optional[str] = None) -> str:
            ...
            # 自定义数据
            custom_data = self.generate_custom_data(custom_input)
...
class Byfuid:
    ...
    def __init__(self):
        self.generator = ByfuidGenerator()
    
    def generate(self, 
                user_data: Optional[str] = None,
                custom_data: Optional[str] = None) -> str:
        """
        生成 BYFUID
        ...
        Returns:
            str: 512字符的BYFUID
        """
        return self.generator.generate_byfuid(user_data, custom_data)
    
    def validate_length(self, byfuid: str) -> bool:
        """
        验证 BYFUID 长度
        ...
        Returns:
            bool: 长度是否有效
        """
        return self.generator.validate_byfuid_length(byfuid)

# 创建全局实例
_byfuid_instance = Byfuid()

def generate_byfuid(user_data: Optional[str] = None, 
                   custom_data: Optional[str] = None) -> str:
    """快速生成 BYFUID"""
    return _byfuid_instance.generate(user_data, custom_data)

def validate_byfuid_length(byfuid: str) -> bool:
    """快速验证 BYFUID 长度"""
    return _byfuid_instance.validate_length(byfuid)

__all__ = ['Byfuid', 'generate_byfuid', 'validate_byfuid_length', 'RUST_AVAILABLE']
__version__ = '0.1.0'